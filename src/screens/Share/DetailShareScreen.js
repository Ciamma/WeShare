...
const DetailShareScreen = ({navigation, set_listening}) => {
    ...
    return (
        <ScrollView style={{backgroundColor:'#6a6161'}} showVerticalScrollIndicator={false}>
                <Text style={[styles.title]}>You shared the contact with:</Text>

                <View style = {styles.root}>

                <View style = {styles.profileCard} >
                        <Image source={card.image} style={styles.profileImage} resizeMode="contain"/>
                        <Text style={styles.profileName}>{card.full_name}</Text>
                        <Text style={styles.cardTitle}>{card.title}</Text>
                        <Text style={styles.info}>It contains <Text style = {{color:'blue'}}>{fieldsCount}</Text> infos</Text>
